...
//    g_hWnd = CreateWindow( "TutorialWindowClass", "Pinguino", WS_OVERLAPPEDWINDOW,
...
HRESULT InitDevice()
{
    ...
    hr = CompileShaderFromFile( "Pinguino.fx", "VS", "vs_4_0", &pVSBlob );
    if( FAILED( hr ) )
    {
        MessageBox( nullptr,
    ...
    hr = CompileShaderFromFile( "Pinguino.fx", "PS", "ps_4_0", &pPSBlob );
    if( FAILED( hr ) )
    {
        MessageBox( nullptr,
                    "The FX file cannot be compiled.  Please run this executable from the directory that contains the FX file.", "Error", MB_OK );
